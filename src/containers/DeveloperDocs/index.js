...
const DeveloperDocs = () => {
  return (
    <div>
      ...
      <h3>Overview</h3>
      <p>The liquidity module is responsible for all the AMM, OrderBook, and Farming Activities. This module implements all the basic features like PairCreation, PoolCreation, AddLiquidity(Deposit), RemoveLiquidity(withdraw), limitOrder, MarketOrder, CancelOrders, Farming, and Unfarming. Multiple applications (apps created using asset module) can leverage the functionality of this module.</p>

      <h3>STATE:</h3>
      <p>
        1. Params - Params stores the global parameters of the liquidity module, these are the parameters that are used regardless of which app (appID) is being served.
      </p>

      <p>
        message Params <br />
        2. GenericParams - GenericParams stores all the parameters used by a specific app, these parameters can be modified by the proposal raised by the gov module via submit-proposal liquidity-param-change. These parameters are basic configurations for the app in the liquidity module. Genericparams are automatically created for the app after its successful registration.
      </p>

      <p>
        Note - App is created via asset module, Please refer x/Asset for more info about the same.
      </p>
      <pre>{`message GenericParams {
   uint64 batch_size
...
`}</pre>
      ...
      <p>
        4. Pool - The pool stores the details about liquidity pools, each pool is unique. A pool cannot be created again with a given pair_id if it already exists in the state. Every pool has its own reserve address which holds the funds provided by liquidity providers.
      </p>
      <pre>{`message Pool {
   uint64 id
   uint64 pair_id
...
`}</pre>
      ...
      <p>
        5. Deposit Request - Requests made to add liquidity in pools are being stored in the DepositRequest state, once the request is executed it gets deleted. These requests are being executed batch-wise.
      </p>
      <pre>{`message DepositRequest {
   uint64 id
   uint64 pool_id
   int64 msg_height
   string depositor
...
`}</pre>
      ...
      <p>
        6. Withdraw Request - Requests made to remove liquidity from pools are being stored in the WithdrawRequest state, once the request is executed it gets deleted. These requests are being executed batch-wise.
      </p>
      <pre>{`message WithdrawRequest {
   uint64 id
   uint64 pool_id
   int64 msg_height
   string withdrawer
...
`}</pre>
      ...
      <p>
        7. Order - Every request made for trading is stored in this order state, it stays until it gets fully executed, or canceled, or until it is expired.
      </p>
      <pre>{`message Order {
   uint64 id
   uint64 pair_id
...
`}</pre>
      ...
      <p>
        8. Active Farmer - The addresses in this state are eligible to receive the farming rewards (swap-fee, internal, external incentivization). Farmed_pool_coin determines the reward ratio of the farmer while rewards calculations.
      </p>
      <pre>{`message ActiveFarmer{
   uint64 app_id
   uint64 pool_id
   string farmer
...
`}</pre>
      ...
      <p>
        9. QueuedFarmer - After farming a farmer stays in this state for one epoch (minimum of all available liquidity epoch types). Once the epoch duration is satisfied for the farmer, this state gets deleted and it is moved into the ActiveFarmer state.
      </p>
      <pre>{`message QueuedCoin{
   cosmos.base.v1beta1.Coin farmed_pool_coin
...
`}</pre>
      ...
      <h3>STATUS:</h3>
      <p>
        1. Request Status - These are not states but they are used in other states which are mentioned further. It can be changed for specific requests throughout their life cycle.
      </p>
      <pre>{`enum RequestStatus {
...
`}</pre>
      ...
      <p>
        2. Order Status - These are not states but they are used in other states which are mentioned further. A single order can have different order statuses depending on the actions being performed on it throughout the life cycle.
      </p>
...
`}</pre>
      ...
      <p>
        3. Order Direction - These are not states but they are used in other states which are mentioned further. Different actions are performed in order depending on the direction of it.
      </p>
      <pre>{`enum OrderDirection {
...
`}</pre>
      ...
      <p>1. MsgCreatePair - Create a pair (market) for trading A Pair can be created in two ways,</p>
      ...
      <p>comdex tx liquidity create-pair [app-id] [base-coin-denom] [quote-coin-denom] –from wallet –chain-id comdex-1 –node domain.com:port</p>
      <pre>{`message MsgCreatePair {
  string creator
  string base_coin_denom
  string quote_coin_denom
  uint64 app_id
...
`}</pre>
      ...
      <p>comdex tx gov submit-proposal create-liquidity-pair [app-id] [base-coin-denom] [quote-coin-denom] –from wallet –chain-id comdex-1 –node domain.com:port</p>

      <p>NOTE - Method (a) has been currently disabled, it will be enabled in future upgrades.</p>

      <p>A pair consists of a base coin and a quote coin and you can think of a pair in an order book. An orderer can request a limit or market order once a pair is created</p>

      <p>For creating a new pair, demons mentioned in base_coin_denom and quote_coin_denom should be allowed by the asset module.</p>

      <p>Pair can be created only with the demons that exist/are whitelisted in the asset module. For more info about the same please refer to the x/Asset section in this doc.</p>
      ...
      <ul>
        <li>
          ...
          <ul>
            ...
            <li>Check that pair isn’t already existed with the given app_id, base_coin_denom, and quote_coin_denom</li>
          </ul>
        </li>
        <li>
      ...
      <p>1. MsgCreatePool - Create a liquidity pool in the existing pair.</p>

      <p>Pool(s) belong to a pair. Therefore, a pair must exist in order to create a pool</p>

      <p>For pool creation, a certain amount of fees needs to be paid (default 200 CMDX). Each pool is unique. At the initial creation of the pool, a fixed amount of max(deposit_coins.Amount) pool tokens is minted and sent to the creator of the pool’s account. The pool coin denom is in the form of pool{"{app_id}-{pool_id}"}.</p>

      <p>Eg. CLI command -</p>

      <p>comdex tx liquidityV1 create-pool [app-id] [pair-id] [deposit-coins] –from wallet –chain-id comdex-1 –node domain.com:port</p>

      <pre>{`message MsgCreatePool {
   string creator
   uint64 pair_id
   repeated cosmos.base.v1beta1.Coin deposit_coins
   uint64 app_id
...
`}</pre>
      ...
      <p>2. MsgDeposit - Deposit coins to a liquidity pool. Deposit uses a batch execution methodology.</p>

      <p>Deposit requests are accumulated in a batch for a predefined period (default is 1 block) and they are executed at the end of the batch. A minimum deposit amount is 1000000 for each denomination.</p>
      ...
      <p>
        comdex tx liquidityV1 deposit [app-id] [pool-id] [deposit-coins] –from wallet –chain-id comdex-1 –node domain.com:port
      </p>
      <pre>{`message MsgDeposit {
   string depositor
   uint64 pool_id
   repeated cosmos.base.v1beta1.Coin deposit_coins 
   uint64 app_id
...
`}</pre>
      ...
      <ul>
        <li>Validation checks that are being done before running deposit logic -
          <ul>
            <li>Check that app_id is valid and that the app with this ID exists in the asset module.</li>
      ...
      <p>3. MsgWithdraw - Withdraw coins from the liquidity pool.</p>
...
`}</pre>
      ...
      <ul>
        <li>Validation checks that are being done before running withdraw logic -
          <ul>
      ...
      <p>4. MsgLimitOrder - Make a limit order.</p>
      ...
      <p>
        comdex tx liquidityV1 limit-order [app-id] [pair-id] [direction] [offer-coin] [demand-coin-denom] [price] [amount] –from wallet –chain-id comdex-1 –node domain.com:port
      </p>

      <pre>{`message MsgLimitOrder {
   string orderer
   uint64 pair_id
   OrderDirection direction
   cosmos.base.v1beta1.Coin offer_coin
...
`}</pre>
      ...
      <p>5. MsgMarketOrder - Make a market order.</p>
      ...
      <p>comdex tx liquidityV1 market-order [app-id] [pair-id] [direction] [offer-coin] [demand-coin-denom] [amount] –from wallet –chain-id comdex-1 –node domain.com:port</p>

      <pre>{`message MsgMarketOrder {
   string orderer
   uint64 pair_id
   OrderDirection direction
   cosmos.base.v1beta1.Coin offer_coin
...
`}</pre>
      ...
      <p>6. MsgCancelOrder - Cancel an order.</p>

      <p>Eg. CLI command -</p>

      <p>comdex tx liquidityV1 cancel-order [app-id] [pair-id] [order-id] –from wallet –chain-id comdex-1 –node domain.com:port</p>
      <pre>{`message MsgCancelOrder {
   string orderer
   uint64 pair_id
   uint64 order_id
   uint64 app_id
...
`}</pre>
      ...
      <p>comdex tx liquidityV1 cancel-all-orders [app-id] [pair-ids] –from wallet –chain-id comdex-1 –node domain.com:port</p>

      <pre>{`message MsgCancelAllOrders {
   string orderer
   uint64 pair_id
   uint64 app_id
 }
`}</pre>
      ...
      <ul>
        <li>Validation checks that are being done before running cancel order logic -
          <ul>
            <li>Check that app_id is valid and an app with this ID exists in the asset module.</li>
          </ul>
        </li>
      ...
      <p>8. MsgFarm - Farm pool tokens to receive farming rewards.</p>

      <p>Eg. CLI command -</p>

      <p>comdex tx liquidityV1 farm [app-id] [pool-id] [pool-coin] –from wallet –chain-id comdex-1 –node domain.com:port</p>
      <pre>{`message MsgFarm {
   uint64 app_id   
   uint64 pool_id
   string farmer
...
`}</pre>
      ...
      <ul>
        <li>Validation checks that are being done before running farm logic -
          <ul>
      ...
      <p>9. MsgUnfarm - Unfarm pool token from the network</p>

      <p>Eg. CLI command -</p>

      <p>comdex tx liquidityV1 unfarm [app-id] [pool-id] [pool-coin] –from wallet –chain-id comdex-1 –node domain.com:port</p>

      <pre>{`message MsgUnfarm {
   uint64 app_id   
   uint64 pool_id
   string farmer
...
`}</pre>
      ...
      <ul>
        <li>Validation checks that are being done before running unfarmlogic -
          <ul>
